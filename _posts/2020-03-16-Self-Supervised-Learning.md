...
### 2.1 通过先验对重建结果进行约束
对于真实图像 x ，它的观测结果为 y ，观测的退化过程定义为 H(·) ，则图像的退化过程可以描述为： $y=H(x)$ 。 而图像复原的任务是找到一种退化过程的逆映射 G(·) ，即： $x=G(y)$ 。 很显然这个逆过程是 ill-posed 的，多以需要引入额外的约束来规范 x ，一种常见的是 MAP 方法，即通过下面的方式解出 x ： $\tilde{x}=arg\max_xp(x|y)=arg\max_xp(y|x)p(x)$ 。 对这个式子计算负对数，可以等价为下面的损失函数：$\min_x \{l_{recons}+\lambda l_x\}$ ， 其中 $l_{recons}$ 为重建loss，对应着 $p(y|x)$ ； $l_x$为对重建后的 x 的正则项, 对应着先验项 $p(x)$ 。  
...
在 “*Double-DIP：Unsupervised Image Decomposition via Coupled Deep-Image-Priors*” 文章中，作者对 DIP 的方法进行了扩展，这与这样的一个模型：$I(x)=m(x)y_1(x)+(1-m(x))y_2(x)$ , 通过两个 DIP 网络，完成了很多 low-level 的任务，如：Segmentation、Transparent Layers Separation、Image Dehazing 。他的方法流程如下图：  
...
对于Physical model，在上文提到的退化过程 $y=H(x)$ ，而要使重建后的 $\tilde{x}$ 与真实的 x 非常接近，那么重建后的 $\tilde{x}$ 经过退化之后也应该与观测图像 y 相符合，即可以有约束： $Loss(y,&space;H(\tilde{x}))$ 。
