...
## 1. 摘要
本论文针对视频去模糊问题，提出了一个简单而有效的深度卷积神经网络模型，该模型主要包括光流估计和潜在清晰帧恢复两个步骤。本文算法首先使用一个深度网络从潜在清晰帧的中间结果中估计光流，进而基于估计出来的光流对潜在清晰帧进行恢复。为了更好地探索视频中的时序信息，本文算法引入时序清晰先验来约束深度卷积神经网络，从而帮助潜在清晰帧的恢复。我们使用级联的训练策略，实现对所提出的深度卷积神经网络模型的端到端训练。在本文中，我们证明，探索视频去模糊的领域信息能够让深度卷积神经网络更加紧致而有效。大量的实验结果表明，所提出的算法在Benchmark数据集和真实场景的视频上都取得SOTA的效果。
...
由于我们的算法需要从潜在清晰帧的中间结果中估计光流，需要引入一个反馈机制，所以我们采用了级联的训练策略，上图是一个 Stage 的总览图。在每个 Stage 中，算法主要包括三个部分：光流估计模块（Optical Flow Estimation Module）、潜在清晰图像重建模块（Latent Image Restoration Module）、以及时序清晰先验（Temporal Sharpness Prior）。且每个 Stages 的参数是共享的，即这个反馈机制不会增加模型的参数量。接下来会对算法的各个部分做出详细的介绍，为了方便论述，我们以相邻的三帧模糊序列作为输入来进行介绍。
