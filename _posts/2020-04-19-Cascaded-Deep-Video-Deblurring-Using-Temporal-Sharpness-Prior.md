...
那我们是不是可以把基于变分模型方法的完善的优化原则引入到基于 CNN 的方法中，同时探索 Domain Knowledge 来让 CNNs 更加紧凑有效呢？
...
### 4.5 算法的局限性
虽然时序清晰先验在视频模糊显著的情况下有效，但是当视频中的所有帧的每一个像素都存在模糊时，时序清晰先验的效果会大打折扣。因为在这些视频中，时许清晰先验不能有效的区分出像素是否清晰。我们在数据集 [24] 中选取了 3 个每帧都模糊的视频，下图是在这 3 个视频上的定量结果，我们主要到使用时序清晰先验并没有提升去模糊的性能。
